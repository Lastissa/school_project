...
class QuizApp(Tk, Question):
    def __init__(self):
        ...
        self.current_question_index = IntVar(value=0) # for tracking the current question index in the quiz_start
        self.current_score_list = StringVar(value="")#track current opt picks "a,b,c,d,c,a" and so on
        self.selected_option = StringVar(value="") # for tracking the selected option in the quiz_start and addi
        ...
        self.selected_course = StringVar(master = self,value=self.courses[0]) #for the dropwown in the welcome page
        self.hours = [f"{x:02d}" for x in range(5)]#4 hour max
        self.selected_hour = StringVar(master = self,value="0")
        ...
        self.selected_min = StringVar(master = self,value="10")
        self.selected_question_lenght = StringVar(master = self,value="00") # the lenght of the question the user want to do
        self.question_option = [f"{i:02d}" for i in range(0, 101, 5)]
        
        self.calculator_is_active = False #To track when the cal is active or not
        self.cal_open__lenght = 0 # traacking the amount of time user open calc
        ...
        self.home_frame = Frame(self, bg= self.bg)
        self.quiz_frame = Frame(self, bg=self.bg, padx=10, pady=12)
    ...
    def build_home_page(self):
        ...
        self.content_wrapper = Frame(self.home_frame, bg="#0B0F19", padx=40, pady=40)#for inner frame so i can del all stuff without delay like the calculator
        self.content_wrapper.pack(fill="both", expand=True)
        #intro label
        lbl_welcome = Label(self.content_wrapper, text="QUIZ APP", font=("Segoe UI", 26, "bold"), fg="#F8FAFC", bg="#0B0F19")
        lbl_welcome.pack(anchor="w", pady=(0, 4))
        #intro words
        lbl_desc = Label(self.content_wrapper, text="A simple quiz app that help access your knowledge on a course", font=("Segoe UI", 11), fg="#64748B", bg=self.bg)
        lbl_desc.pack(anchor="w", pady=(0, 25))
        #the divider for decoration
        divider = Frame(self.content_wrapper, height=1, bg="#1F2937")
        divider.pack(fill="x", pady=(0, 30))
        #Frame for selecting course, duration and question
        data_collection_frame = Frame(self.content_wrapper, bg="#111827", padx=35, pady=35, highlightbackground="#1F2937", highlightthickness=1)
        data_collection_frame.pack(fill="x", pady=(0, 35))
        # 1. Course Selection Dropdown Section
        lbl_course_title = Label(data_collection_frame, text="SELECT COURSE", font=("Segoe UI", 9, "bold"), fg="#94A3B8", bg="#111827")
        lbl_course_title.pack(anchor="w", pady=(0, 8))
        #courses option
        course_menu = OptionMenu(data_collection_frame, self.selected_course, *self.courses, command = None)
        self.style_option_menu(course_menu)
        course_menu.pack(fill="x", pady=(0, 25))
        # 2. Time Allocation Dropdown Section
        lbl_duration_title = Label(data_collection_frame, text="LIMIT DURATION", font=("Segoe UI", 9, "bold"), fg="#94A3B8", bg="#111827")
        lbl_duration_title.pack(anchor="w", pady=(0, 8))
        #for hour and course
        picker_row = Frame(data_collection_frame, bg="#111827")
        picker_row.pack(anchor="w", fill="x")
        #for the hour selector
        hour_menu = OptionMenu(picker_row, self.selected_hour, *self.hours)
        self.style_option_menu(hour_menu)
        hour_menu.pack(side="left")
        lbl_colon = Label(picker_row, text=":", font=("Segoe UI", 14, "bold"), fg="#4B5563", bg="#111827")
        lbl_colon.pack(side="left", padx=12)
        #for the minute selector
        min_menu = OptionMenu(picker_row, self.selected_min, *self.minutes)
        self.style_option_menu(min_menu)
        min_menu.pack(side="left")
        #duration suffix
        lbl_suffix = Label(picker_row, text="HH : MM", font=("Segoe UI", 10, "bold"), fg="#4B5563", bg="#111827")
        lbl_suffix.pack(side="left", padx=15)
        #question_count label
        question_count_lbl = Label(picker_row, text="question lenght".upper(), font=("Segoe UI", 9, "bold"), fg="#94A3B8", bg="#111827")
        question_count_lbl.pack(anchor="e")
        #question amount selector
        question_count = OptionMenu(picker_row, self.selected_question_lenght, *self.question_option)
        self.style_option_menu(question_count)
        question_count.pack(anchor="e")
        #start quiz button
        self.btn_start = Button(self.content_wrapper, text="Start", font=("Segoe UI Semibold", 12), bg="#2563EB", fg="#FFFFFF", padx=35, pady=12, command=self.start_quiz)
    ...
    def start_quiz(self):
        ...
        if duration == 0 or int(self.selected_question_lenght.get()) == 0:
            try: # incase this is the first time THE ERROR IS SHOWN as pack_forget wont work yet so the except should br used
                ...
                self.dur_err_warn = Label(self.content_wrapper, text="INCOMPLETE PARAMETERS!!!", fg="#ff8888", bg = self.bg, font=("Segoe UI", 13, "bold"),)
                self.dur_err_warn.pack()
                self.dur_err_warn.after(2500,lambda: [self.dur_err_warn.destroy()])
            except:
                self.dur_err_warn = Label(self.content_wrapper, text="INCOMPLETE PARAMETERS!!!", fg="#ff8888", bg = self.bg, font=("Segoe UI", 11),)
                self.dur_err_warn.pack()
                self.dur_err_warn.after(2500,lambda: [self.dur_err_warn.destroy()])
        ...
        if self.questionFilter() == False:
            try: # incase this is the first time THE ERROR IS SHOWN as pack_forget wont work yet so the except should br used
                self.dur_err_warn.pack_forget()
                self.dur_err_warn = Label(self.content_wrapper, text= f"Errror, Max allowed question is  {len(self.allQuestionBeforeFilter)}", fg="red", bg = self.bg, font=("Segoe UI", 11),)
                self.dur_err_warn.pack()
                self.dur_err_warn.after(2500,lambda: [self.dur_err_warn.destroy()])
            except:
                self.dur_err_warn = Label(self.content_wrapper, text= f"Error, Max allowed question is  {len(self.allQuestionBeforeFilter)}", fg="red", bg = self.bg, font=("Segoe UI", 11),)
                self.dur_err_warn.pack()
                self.dur_err_warn.after(2500,lambda: [self.dur_err_warn.destroy()])
            return
    ...
    def calculator_func(self):
        print(f"cal is active? {self.calculator_is_active}")
        self.cal_open__lenght += 1 #increment the cal open time lenght
        print(f"Opened the cal {self.cal_open__lenght}")
        # if the call time is to a certainpoint, dont allow cal to opne again
        if self.cal_open__lenght >= 5:
            #notiy the usser dem don reach limit
            self.calc.config(text="max limit reached", bg="red", disabledforeground="white",state="active")
            self.calc.after(3000, lambda : [self.calc.config(text="calc", bg="green", fg= "white")])
            return
        
        #bring up th calc
        CoverUp(width_pos=self.width, height_pos = 0)
        

    def quiz_first_frame(self):
        self.quiz_header_frame = Frame(self.quiz_frame, bg= self.bg)
        self.quiz_header_frame.pack(fill="x", pady=10, padx=10)

        #Left: Course Title
        self.course_lbl = Label(self.quiz_header_frame, text=self.selected_course.get(), font=("Segoe UI", 15, "bold"), bg=self.bg, fg="white", anchor="w")
        self.course_lbl.pack(side="left", padx=10)
        #cal btn
        self.calc = Button(self.quiz_header_frame, text="calc", font=("Segoe UI", 9), bg="green", fg="white", padx=10, pady=5 ,bd=0, command=lambda : self.calculator_func())
        self.calc.pack(side="left", padx=10)
        #right: Duration Title
        self.duration_countdown_display()
    
    def quiz_second_frame(self):
        self.quiz_questioon_frame = Frame(self.quiz_frame, bg= self.bg)
        self.quiz_questioon_frame.pack(fill="both", expand=True, padx=10, pady=20)
        self.quiz_questioon_label = Label(self.quiz_questioon_frame, text=self.question_pool[self.current_question_index.get()][0], font=("Segoe UI", 12), bg=self.bg, fg="white", justify="left", wraplength= self.width - 40)
    ...
    def quiz_third_frame(self):
        self.quiz_option_frame = Frame(self.quiz_frame, bg= self.bg)
        self.quiz_option_frame.pack(fill="x", padx=10, pady=10)
        #the options radio buttons
        #(1)
        self.quiz_opt_btn_1 = Radiobutton(
        ...
        #(2)
        self.quiz_opt_btn_2 = Radiobutton(
                self.quiz_option_frame,
        ...
        #(3)
        self.quiz_opt_btn_3 = Radiobutton(
                self.quiz_option_frame,
        ...
        self.quiz_opt_btn_3.pack(anchor="w", pady=5, padx=10)
         #(4)
        self.quiz_opt_btn_4 = Radiobutton(
                self.quiz_option_frame,
    ...
    def quiz_fourth_frame(self):
        ...
        self.quiz_footer_frame = Frame(self.quiz_frame, bg= self.bg)
        self.quiz_footer_frame.pack(fill="x", expand=True)

        #prev
        self.footer_button_prev = Button(self.quiz_footer_frame, text="Previous", font=("Segoe UI", 11), bg="#aaaaaa", fg="white", padx=10, pady=5 ,bd=0, command=lambda: self.prevQuiSubmitNext("Prev"))
        ...
        #quit
        self.footer_button_quit = Button(self.quiz_footer_frame, text="Quit", font=("Segoe UI", 11), bg="#DC2626", fg="white", padx=10, pady=5 ,bd=0, command=lambda: self.prevQuiSubmitNext("Quit"))
        ...
        #submit
        self.footer_button_submit = Button(self.quiz_footer_frame, text="Submit", font=("Segoe UI", 11), bg="#16A34A", fg="white", padx=10, pady=5 ,bd=0, command=lambda: self.prevQuiSubmitNext("Submit"))
        ...
        #next
        self.footer_button_next = Button(self.quiz_footer_frame, text="Next", font=("Segoe UI", 11), bg="#173E94", fg="white", padx=10, pady=5 ,bd=0, command=lambda: self.prevQuiSubmitNext("Next"))
    ...
    def quiz_fifth_frame(self):
        self.quiz_footer_second_frame = Frame(self.quiz_frame, bg= self.bg)
        self.quiz_footer_second_frame.pack(anchor="s", pady=10, padx=10, fill="x", expand=True)
        self.current_question_footer = Label(self.quiz_footer_second_frame, text=f"Question {self.current_question_index.get() + 1} of {int(self.selected_question_lenght.get())}", font=("Segoe UI", 14, "bold"), bg=self.bg, fg="#94A3B8")
    ...
    def prevQuiSubmitNext(self, action):
        if action == "Prev":
            ...
            tkinter.messagebox = tkinter.messagebox.askyesno("Quit Quiz", "Are you sure you want to quit?\nCurrent Data will be lost!")
    ...
    def loadAnalysis(self):
        ...
        #widget drawup
        self.analysis_frame = Frame(self, bg=self.bg)
        self.analysis_frame.pack(expand=True, fill="both", padx=10, pady=10)

        #the  report label
        report = Label(self.analysis_frame, text = "ANALYSIS",font=("Segoe UI", 26, "bold"), fg="#F8FAFC", bg="#0B0F19")
        ...
        inner_frame = Frame(self.analysis_frame, bg= self.bg)
        inner_frame.pack(expand=True, fill="both")
        
        for index, value in enumerate(information_to_display.keys()):
            btn_left = Label(inner_frame, text=value, font=("Segoe UI", 11), bg = self.bg, fg="white", pady=1, anchor="w")
            btn_left.grid(row=index, column=0,pady=5, padx=10, sticky="ew")
            inner_frame.grid_rowconfigure(index, weight=1)
            inner_frame.grid_columnconfigure(0, weight=1)
            
            btn_right = Label(inner_frame, text=information_to_display[value], justify="right",font=("Segoe UI", 13, "bold"), bg = self.bg, fg="white", pady=1, anchor="center")
            btn_right.grid(row=index, column=1,pady=5, sticky="ew")
            inner_frame.grid_rowconfigure(index, weight=1)
            inner_frame.grid_columnconfigure(1, weight=1)
        
        #the analysis btn
        analysis_btn = Button(self.analysis_frame, text="Q SUMMARY", font=("Segoe UI Semibold", 12),bg="#2563EB",  fg="#FFFFFF",padx=35, command= self.eachQuestionAnalysis)
        analysis_btn.pack(fill="x", expand=True, padx=40)
        
        #the speech button - speed remark
        word_remark = Label(self.analysis_frame, text=word_remark_text,font=("Segoe UI", 11, "italic"), fg="#AFCFF0", bg="#0B0F19")
        word_remark.pack(fill="x", expand=True)
        
        #the home button
        home = Button(self.analysis_frame, text="HOME", font=("Segoe UI Semibold", 12),bg="#2563EB",  fg="#FFFFFF",padx=35,  command= lambda: self.home(old_frame = self.analysis_frame, new_frame_method=self.build_home_page))
    ...
    def eachQuestionAnalysis(self):
        ...
        self.deepAnalysisFrame = Frame(self, padx=10, pady=10, bg=self.bg)
        ...
        #question label
        self.quiz_questioon_label = Label(self.deepAnalysisFrame, text=self.question_pool[self.current_question_index.get()][0], font=("Segoe UI", 12), bg=self.bg, fg="white", justify="left", wraplength= self.width - 40, pady=15)
        self.quiz_questioon_label.pack(fill="x")
        self.quiz_questioon_label.bind("<Configure>", lambda event, extra = self.quiz_questioon_label: self.configureQuestionWidth(widget=  extra, event=event))
        
        #for the options radio buttons
        self.quiz_option_frame = Frame(self.deepAnalysisFrame, bg= self.bg)
        ...
        self.correct_answer = Label(self.deepAnalysisFrame, text= f"Option {self.question_pool[self.current_question_index.get()][-1].upper()} is the correct answer", fg="white", font=("Segoe UI", 11),)
        self.deepAnalysisRightAns()
        ...
        self.quiz_footer_frame = Frame(self.deepAnalysisFrame, bg= self.bg)
        self.quiz_footer_frame.pack(fill="x", expand=True)
        ...
        self.footer_button_quit = Button(self.quiz_footer_frame, text="Back", font=("Segoe UI", 11), bg= "#173E94", fg="white", padx=10, pady=5 ,bd=0, command=lambda: self.home(old_frame=self.deepAnalysisFrame, new_frame_method=self.loadAnalysis))
        self.footer_button_quit.grid(row=0, column= 1, padx=10, pady=10, sticky="ew")
        self.quiz_footer_frame.grid_rowconfigure(0, weight = 1)
        self.quiz_footer_frame.grid_columnconfigure(1, weight= 1)
         
        #home
        self.footer_button_submit = Button(self.quiz_footer_frame, text="Home", font=("Segoe UI", 11), bg="#DC2626", fg="white", padx=10, pady=5 ,bd=0, command=lambda: self.home(old_frame=self.deepAnalysisFrame, new_frame_method=self.build_home_page))
        self.footer_button_submit.grid(row=0, column= 2, padx=10, pady=10, sticky="ew")
        self.quiz_footer_frame.grid_rowconfigure(0, weight = 1)
        self.quiz_footer_frame.grid_columnconfigure(2, weight= 1)
        ...
        self.quiz_footer_second_frame = Frame(self.deepAnalysisFrame, bg= self.bg)
        self.quiz_footer_second_frame.pack(anchor="s", pady=10, padx=10, fill="x", expand=True)
        self.current_question_footer = Label(self.quiz_footer_second_frame, text=f"Question {self.current_question_index.get() + 1} of {int(self.selected_question_lenght.get())}", font=("Segoe UI", 14, "bold"), bg=self.bg, fg="#94A3B8")
        self.current_question_footer.pack(anchor="e")
